...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const layoutOptions: LayoutOptions = {
      ...
      qrSize,
      accountGroom: hasAccount ? (fields.account_groom ?? '') : '',
      accountBride: hasAccount ? (fields.account_bride ?? '') : '',
      groom,
      bride,
      deceasedStyle,
    ...
    const { error: insertError } = await admin.from('orders').insert({
      ...
      has_map: hasMap,
      has_qr: hasQr,
      // 실제 문구는 pages에 들어가고, 이 컬럼은 관리자 참고용 사본이다
      account_text: hasAccount ? [fields.account_groom, fields.account_bride].filter(Boolean).join('\n\n') || null : null,
      orientation,
