...
9. **캔버스에 떠 있는 버튼(×·▲▼)과 회전 손잡이는 서로 닿을 수 있다.** 요소를 조금만 돌려도
   바운딩 박스가 커져 버튼줄이 손잡이 위로 올라온다. 지금은 `rotateAnchorOffset`(20) + 손잡이
   반지름(11)보다 `.selection-toolbar`의 간격(40px)을 크게 잡아 막아 뒀다 — **셋 중 하나를
   고치면 나머지도 같이 봐야 한다.** 각도를 5°씩 훑어 사각형 겹침을 재면 확실하다.
   회전 손잡이는 `anchorStyleFunc`로 rotater만 동그란 회전 아이콘(인라인 SVG)으로 바꿔 둔다.
...
원본 **파일명만** 바뀌었으면 카탈로그의 `label`만 고친다 — `slug`는 저장된 시안이 참조하니 그대로 둔다.
파일명에 번호가 없으면 이름으로는 짝을 못 지으니 **그림 비교**로 찾는다(잘라낸 뒤 32px로 줄여
픽셀 차이 + 종횡비). 웨딩 3D 37개를 이렇게 맞췄는데 짝과 2순위 차이가 20배 넘게 벌어졌다.
...
- 고객: 자동저장(1.2초 디바운스 + 3분 안전망 — 저장이 실패해도 다시 올린다) + 임시저장 버튼 + 탭 이탈 시 저장, 시안 미리보기(전 면),
...
- **템플릿 패널**(꾸미기와 레이어 사이): 미리 만든 아이콘 세트를 **지금 보고 있는 면**에
  ...
  **적용 취소**는 그 면에서 uid가 `tpl-<템플릿id>-<번호>-<순번>`인 아이콘만 한 번에 걷어 낸다.
  ...
  확정 PDF로도 옮길 수 있다 — 안에 면당 1889×1299 PNG가 그대로 들어 있어(FlateDecode +
  PNG predictor) 꺼내서 같은 방식으로 재면 된다. 겹쳐 그린 장식은 색이나 세로줄 밀도로 가른다.
  **주의: sharp는 `trim`을 `extract`보다 먼저 적용한다** — 조각을 떼어 여백을 지우려면
  extract → toBuffer → 새 sharp에서 trim, 두 단계로 나눠야 엉뚱한 영역을 비교하지 않는다.
