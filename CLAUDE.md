...
11. macOS 파일명은 **NFD**로 저장된다. 한글 파일명을 코드의 NFC 문자열과 비교하려면
...
- **템플릿 패널**(꾸미기와 레이어 사이): 미리 만든 아이콘 세트를 앞면에 **낱개 아이콘**으로
  얹는다. 통이미지가 아니라 얹은 뒤에도 하나씩 편집되고, 기존 요소는 건드리지 않고 더하기만
  한다. 얹을 때 글자(이름·생년월일·자막)를 전부 아이콘 위로 올린다(글자는 주문마다 달라서
  템플릿에 넣지 않는다). 미리보기도 `tintImage`로 칠해 캔버스와 같은 색으로 보여 준다.
